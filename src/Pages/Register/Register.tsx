...
const Register = () => {
    ...
    const [userType, setUserType] = React.useState<string>(USER_TYPES[0].name as string);
    const handleUserTypeChange = (event: SelectChangeEvent) => setUserType(event.target.value);
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        Axios.post('/register', userData)
            .then(response => {
                ...
                setUser(newUser);
                sessionStorage.setItem('User', JSON.stringify(newUser as User));
                navigate('/dashboard');
            })
            .catch(resonse => {
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12} sm={6}>
                                ...
                                >
                                   {
                                        USER_TYPES.map(type => {
                                            return <MenuItem value={type.name as string} key={type.id as number}>{type.name}</MenuItem>
                                        })
                                   }
